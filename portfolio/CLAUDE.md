...
White translucent surfaces over a coloured background, near-black text, Apple blue accent (branch `portfolio_theme`).
...
- **Colour roles flipped**, so two tokens were renamed: `--clr-light` → `--clr-text-strong` (near-black, for headings/bold) and `--clr-dark` → `--clr-on-accent` (white, for text on a blue fill). `--clr-white` is for things that must be literally white regardless of theme (the toggle knob). `--clr-accent` (#0071e3) is the *fill* colour — white on it is 4.7:1; `--clr-accent-soft` (#0066cc) is for small blue text **on** glass, which is the case that fails if you use the brighter blue.
